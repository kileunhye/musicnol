...
const state = {
  ...
  score: 0, studentNumber: '', selectedAnswer: '', listeningComplete: false, wrong: false, celebration: false, quizStage: 1, syncHistory: [],
...
function enterStudent() {
  ...
  state.studentNumber = number; state.score = getRecords().find(record => record.studentNumber === number)?.score || 0; state.listeningComplete = false; state.wrong = false; state.celebration = false; state.selectedAnswer = ''; state.usedHints = []; state.hintOrder = []; renderStudent();
}
...
function autoSyncLyrics() { const c = state.teacherContent; const start = Number(c.firstPhraseStart); const end = Number(c.firstPhraseEnd); const length = end - start; if (!Number.isFinite(length) || length <= 0) return alert('첫 프레이즈의 시작과 끝을 먼저 지정해주세요.'); pushSyncHistory(); c.lyrics.forEach((line, index) => { line.start = Number((start + length * index).toFixed(2)); line.end = Number((start + length * (index + 1)).toFixed(2)); }); renderSync(); }
