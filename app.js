...
const state = {
  ...
  score: 0, studentNumber: '', selectedAnswer: '', listeningComplete: false, wrong: false, quizStage: 1,
...
function renderSync() {
  ...
  document.querySelectorAll('.stage-button').forEach(button => button.addEventListener('click', () => { state.quizStage = Number(button.dataset.stage); renderSync(); }));
...
function bindActions() { document.querySelectorAll('[data-action]').forEach(button => button.addEventListener('click', e => { e.preventDefault(); const a = button.dataset.action; if (a === 'home') goHome(); if (a === 'teacher') renderTeacher(); if (a === 'student') renderStudentEntry(); if (a === 'student-entry') renderStudentEntry(); if (a === 'enter-student') enterStudent(); if (a === 'to-sync') renderSync(); if (a === 'mark-line') markCurrentLine(); if (a === 'set-first-start') setFirstPhrasePoint('start'); if (a === 'set-first-end') setFirstPhrasePoint('end'); if (a === 'auto-sync') autoSyncLyrics(); if (a === 'add-intro') addIntroSegment(); if (a === 'reset-sync') { state.teacherContent.lyrics.forEach((line, i) => { line.start = i * 4; line.end = i * 4 + 4; }); renderSync(); } if (a === 'save-content') saveContent(); if (a === 'finish-listening') finishListening(); if (a === 'submit-lyric') submitLyric(); })); if (document.querySelector('.sync-setup') && !document.querySelector('[data-action="add-intro"]')) { document.querySelector('.sync-setup').insertAdjacentHTML('beforeend', '<button class="btn btn-secondary" data-action="add-intro">첫 가사 앞에 전주 추가</button>'); document.querySelector('[data-action="add-intro"]').addEventListener('click', addIntroSegment); } }

function setFirstPhrasePoint(point) { const audio = document.querySelector('#sync-audio'); if (!audio) return; state.teacherContent.firstPhraseStart = point === 'start' ? audio.currentTime : (state.teacherContent.firstPhraseStart ?? 0); state.teacherContent.firstPhraseEnd = point === 'end' ? audio.currentTime : (state.teacherContent.firstPhraseEnd ?? 4); renderSync(); }
function autoSyncLyrics() { const c = state.teacherContent; const start = Number(c.firstPhraseStart); const end = Number(c.firstPhraseEnd); const length = end - start; if (!Number.isFinite(length) || length <= 0) return alert('첫 프레이즈의 시작과 끝을 먼저 지정해주세요.'); c.lyrics.forEach((line, index) => { line.start = Number((start + length * index).toFixed(2)); line.end = Number((start + length * (index + 1)).toFixed(2)); }); renderSync(); }
function addInstrumentalSegment(index) { const c = state.teacherContent; const source = c.lyrics[index]; const start = source?.end ?? (source?.start || 0) + 4; c.lyrics.splice(index + 1, 0, { text: '♪ 간주 / 가사 없음', noLyrics: true, start, end: start + 4 }); if (c.quizLineIndex > index) c.quizLineIndex += 1; renderSync(); }
function addIntroSegment() { const c = state.teacherContent; const end = Number(c.firstPhraseStart); if (!Number.isFinite(end) || end <= 0) return alert('먼저 첫 가사 시작 시점을 지정해주세요.'); if (c.lyrics[0]?.noLyrics && c.lyrics[0].intro) { c.lyrics[0].end = end; return renderSync(); } c.lyrics.unshift({ text: '♪ 전주 / 가사 없음', noLyrics: true, intro: true, start: 0, end }); c.quizLineIndex = (c.quizLineIndex ?? 0) + 1; c.quizStages = getQuizStages(c).map(item => ({ ...item, lyricIndex: item.lyricIndex + 1 })); renderSync(); }

function renderSync() {
  if (document.querySelector('#lyrics')) state.teacherContent = getDraft(); const c = state.teacherContent; c.quizStages ??= [{ stage: 1, lyricIndex: c.quizLineIndex ?? 0 }]; c.firstPhraseStart ??= c.lyrics[0]?.start || 0; c.firstPhraseEnd ??= c.lyrics[1]?.start || c.firstPhraseStart + 4;
  const phraseLength = Math.max(0, c.firstPhraseEnd - c.firstPhraseStart);
  app.innerHTML = `<a href="#" class="back" data-action="teacher">← 음악 등록으로</a><div class="section-head"><div><div class="eyebrow">Teacher studio / 02</div><h2>가사 싱크와 자동 배치</h2><p>간주가 끝나는 첫 가사 시작점과 첫 프레이즈의 길이를 지정하면 다음 가사를 같은 박자로 배치합니다.</p></div><span class="tag">${c.lyrics.length}줄 등록됨</span></div><section class="panel"><div class="stage-tabs"><strong>문제 단계</strong>${[1,2,3].map(stage => `<button class="btn ${state.quizStage === stage ? 'btn-primary' : 'btn-secondary'} stage-button" data-stage="${stage}">${stage}단계</button>`).join('')}<span class="small">${state.quizStage}단계에 출제할 가사를 아래에서 지정하세요.</span></div><div class="sync-layout"><div><div class="audio-box"><h3>${escapeHTML(c.title)}</h3><p class="small">${escapeHTML(c.fileName || '음원 미선택 · 데모 타이밍으로 미리보기')}</p><audio id="sync-audio" controls ${c.audioUrl ? `src="${c.audioUrl}"` : ''}></audio><div class="time-readout" id="sync-time">00:00.0</div><div class="sync-controls"><button class="btn btn-primary" data-action="set-first-start">현재 시점 → 첫 가사 시작</button><button class="btn btn-secondary" data-action="set-first-end">현재 시점 → 첫 프레이즈 끝</button></div><div class="sync-setup"><div><span>첫 시작</span><strong>${formatTime(c.firstPhraseStart)}</strong></div><div><span>프레이즈 길이</span><strong>${phraseLength.toFixed(1)}초</strong></div><button class="btn btn-ghost" data-action="auto-sync">같은 박자로 전체 배치</button></div></div><div class="notice" style="margin-top:14px">먼저 간주가 끝나는 순간에 <strong>첫 가사 시작</strong>을 누르고, 첫 가사가 끝나는 순간에 <strong>첫 프레이즈 끝</strong>을 누르세요. 그 후 자동 배치를 실행합니다.</div></div><div><h3>가사 타임라인</h3><div class="lyric-lines">${c.lyrics.map((line, i) => `<div class="lyric-row ${i === activeQuizIndex(c) ? 'active' : ''} ${line.noLyrics ? 'no-lyrics' : ''}" data-index="${i}"><input class="line-start" type="number" step="0.1" value="${line.start ?? 0}" /><span>${escapeHTML(line.text)}</span><button class="btn btn-ghost quiz-line-button" data-index="${i}" ${line.noLyrics ? 'disabled' : ''}>${line.noLyrics ? '가사 없음' : (i === activeQuizIndex(c) ? `${state.quizStage}단계 지정됨` : `${state.quizStage}단계 지정`)}</button><button class="btn btn-secondary instrumental-button" data-index="${i}">뒤에 간주 추가</button></div>`).join('')}</div></div></div><div class="actions"><button class="btn btn-secondary" data-action="student">학생 화면 미리보기</button><button class="btn btn-primary" data-action="save-content">싱크 저장하기</button></div></section>`;
  bindActions(); const audio = document.querySelector('#sync-audio'); audio.addEventListener('timeupdate', () => { document.querySelector('#sync-time').textContent = formatTime(audio.currentTime); highlightLyric(audio.currentTime); });
  document.querySelectorAll('.line-start').forEach(input => input.addEventListener('change', e => { const i = Number(e.target.closest('.lyric-row').dataset.index); c.lyrics[i].start = Number(e.target.value); c.lyrics[i].end = c.lyrics[i + 1]?.start || c.lyrics[i].start + phraseLength; }));
